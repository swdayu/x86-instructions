...
通常，在 64 位模式下，偏移和立即数不会扩展到 64 位。它们仍然限制为 32 位，并在有效地址
...
ENTER 指令创建一个与块结构语言中通常使用的作用域规则兼容的栈帧。在块结构语言中，过程的
作用域是它能够访问的变量集合。不同语言的作用域规则可能有所不同。它们可能基于过程的嵌套、
程序被划分为单独编译的文件，或其他某种模块花方案。

ENTER 有两个操作数，第一个操作数指定为被调过程在栈上保留的字节数，用于动态存储。动态存储
是为过程创建变量分配的内存，也成为自动变量。第二个参数是过程的语法嵌套级别（0到31）。嵌套
级别是过程在过程调用层次结构中的深度。

语法嵌套级别决定了从前面的帧复制多少帧指针到新的栈帧中。一个过程用来访问其他过程变量的
帧指针集合称为显示区。显示区中的第一个帧指针是一个先前帧指针，这个指针被 LEAVE 指令用来
撤销 ENTER 指令的效果，通过丢弃当前的栈帧。

在 ENTER 指令为过程创建显示区后，它通过减少栈指针为过程分配动态局部变量，其大小由第一个
参数指定。为了允许过程访问显示区，ENTER 指令使用 EBP 寄存器指向显示区的第一个帧指针。
ENTER 指令可以以两种方式使用：嵌套和非嵌套。如果语法级别为0，则使用非嵌套形式，非嵌套形
式将 EBP 寄存器的内容压入栈，将 ESP 寄存器的内容复制到 EBP 寄存器，并从 ESP 寄存器的
内容减去第一个操作数以分配动态存储。非嵌套与嵌套形式的不同之处在于不复制栈指针。当第二个
参数不为零时，使用 ENTER 指令的嵌套形式。

以下伪代码显示了 ENTER 指令的定义，STORAGE 是为局部变量分配的动态存储字节数，LEVEL 是
语法嵌套级别： ::
    ...
    ESP := ESP - STORAGE;
            ...
            | 函数返回地址 |
    level 0 | 原来旧的EBP  | push ebp               <-- ebp_0   非嵌套形式
            | 局部变量     | mov ebp,esp
            | 局部变量     | sub esp,N
            | ...          |
            | 保护寄存器   |
            | 保护寄存器   |
            | ...          |
    level 1 | ebp_0        | push ebp  mov eax,esp   <-- ebp_1   嵌套形式
            | ebp_1        | push eax
            | 局部变量     | mov ebp,eax
            | 局部变量     | sub esp,N
            | ...          |
    level 2 | ebp_1        | push ebp  mov eax,esp   <-- ebp_2   嵌套模式
            | ebp_1        | push [ebp-4]
            | ebp_2        | push eax
            | 局部变量     | mov ebp,eax
            | 局部变量     | sub esp,N
            | ...          |
    level 3 | ebp_2        | push ebp  mov eax,esp   <-- ebp_3   嵌套模式
            | ebp_1        | push [ebp-4]
            | ebp_2        | push [ebp-8]
            | ebp_3        | push eax
            | 局部变量     | mov ebp,eax
